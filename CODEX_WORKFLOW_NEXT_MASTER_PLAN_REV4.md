...
**Ревизия:** **4.0 — Evidence-grounded Desktop-first architecture**
...
**Current upstream reference:** `viettran-edgeAI/codex_workflow` `experiment/beta-install-prompt` **v1.1.14**, SHA `a224f32c423ef56be322de160d5440bba0a786b2`
**Executed platform baseline:** Windows 11 build 26200, Desktop `26.901.1978.0`, CLI `0.153.4`
**PH-00 outcome:** **PASS_WITH_AMENDMENTS**

> This revision consolidates the original Desktop-first design, trace-based eval architecture, the `v1.1.13/v1.1.14` upstream review, and the actual PH-00 capability evidence. Earlier amendment files are historical only; this document is the new architectural source of truth.
>
...
Implementation order is controlled by `CODEX_WORKFLOW_NEXT_ROADMAP_REV2.md`. The currently executable phase plan is `CODEX_WORKFLOW_PH01_IMPLEMENTATION_PLAN.md`.
...
| Area | IDs |
|---|---|
| Product | `GOAL-01..08`, `NOGO-01..14` |
| Evidence baseline | `CAP-01..14` |
| Principles | `PRN-01..18` |
...
| Context | `CTX-01..12` |
| Policy | `POL-01..13` |
| State/MCP/Hooks | `ARC-09..16`, `HK-01..08`, `API-01..05` |
| Board | `UX-01..18` |
| Security | `SEC-01..12` |
| Evals | `OBS-01..18` |
...
L4 repo code/docs/ADR     durable truth
...
Completion transitions are mechanically validated against required evidence/decisions.

<a id="prn-10"></a>
## [PRN-10] Semantic role != model

Roles remain stable; model/effort mappings evolve through configuration.
...
No implicit expansion of scope, authority, concurrency, retry budget or model cost tier.
...
Role and model are separate. Effective provider model is recorded only if publicly observable; otherwise retain `requestedModel/requestedEffort` with `effective=unknown`.
...
This can narrow but never expand native/user/project authority.
...
Purpose: choose smallest useful topology.
...
V1 logic remains decision-table/heuristic, not scalar optimizer.
...
Current primary thread. Final decisions/integration/claims.

<a id="agt-02"></a>
## [AGT-02] Context Companion

Fresh, read-only, conditional. Maintains hot local repo context and returns Context Delta. Does not become durable memory.

<a id="agt-03"></a>
## [AGT-03] Investigator

External/current research, source-linked. Separate from Companion because trust/freshness semantics differ.

<a id="agt-04"></a>
## [AGT-04] Builder

Bounded implementation worker. Initial production path uses explicit fresh context and self-contained envelope.

<a id="agt-05"></a>
## [AGT-05] Verifier

Fresh independent verifier when risk/evidence warrants it.

<a id="agt-06"></a>
## [AGT-06] Docs Steward

Optional documentation/handoff materializer. May document already-settled decisions; cannot create/alter architectural decisions or ADRs autonomously. **Never owns usage collection.**
...
<a id="pol-09"></a>
## [POL-09] Model profiles are semantic

`efficient/balanced/deep/critical`; current model mapping is configuration.
...
SQLite = operational state/cache/evidence metadata. Repo docs/code/ADR = durable product/project truth.
...
<a id="wp-014"></a>
### [WP-014] Minimal AGENTS + docs map

Permanent invariants and references only.
...
- three foundational Skills have non-overlapping triggers;
...
Fresh read-only Companion, `fork_turns=none`, Context Index/fingerprint/hydration/delta. Economic value not claimed until PH-06.
...
- semantic profiles;
- fresh Builder/Investigator;
- bounded recovery;
- temporary DecisionBatch candidate.

Before parallel writers: native managed-worktree write isolation smoke.
...
Risk classifier, evidence normalization, native review integration, post-run scope validation, completion validator.
...
Security review, cross-platform smoke, migration/coexistence, SBOM/provenance/docs/public benchmark methodology.
...
<a id="adr-22"></a>
## [ADR-22] Docs Steward cannot own architectural decisions or telemetry
Accepted.
